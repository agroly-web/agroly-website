...
class PenjualanBarangForm(forms.ModelForm):
...
class PenerimaanPiutangForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            "akun_kas_setoran": forms.Select(attrs={"class": "form-select form-select-sm"}),
            "customer": forms.Select(attrs={"class": "form-select form-select-sm", "id": "id_customer"}),
            "faktur_no": forms.TextInput(attrs={"class": "form-control form-control-sm"}),
            ...
            "jumlah_terima": forms.NumberInput(attrs={"class": "form-control form-control-sm", "step": "0.01"}),
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.instance.pk:
            self.fields["akun_kas_setoran"].initial = Akun.objects.filter(kode_akun="11102").first()
